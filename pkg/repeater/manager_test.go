...
func TestSingleActiveStreamEnforcement(t *testing.T) {
	...
	m.ProcessPacket(r1.Callsign(), r1.Callsign(), addr1, "YSFD", 100)
	if !r1.IsTalking() {
		t.Fatalf("expected r1 to be talking")
	}

	// Second repeater attempts to talk but should be ignored
	m.ProcessPacket(r2.Callsign(), r2.Callsign(), addr2, "YSFD", 100)
	if r2.IsTalking() {
	...
	m.ProcessPacket(r2.Callsign(), r2.Callsign(), addr2, "YSFD", 50)
	if !r2.IsTalking() {
...
func TestMuteOnTalkMaxDuration(t *testing.T) {
	...
	m.ProcessPacket(r1.Callsign(), r1.Callsign(), addr1, "YSFD", 50)
